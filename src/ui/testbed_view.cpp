...
TestbedView::TestbedView(const DistroboxInfo& info, std::function<void()> on_refresh)
    ...
    auto icon_tv = TextViewBuilder::create()->text("📦")->h1()->build();
    icon_tv->set_margin(0, 0, 14, 0);
    status_row->add_view(icon_tv);
    ...
    // =========================================================================
    auto list_hdr = TextViewBuilder::create()
        ->text("Configured Testbeds")
        ->h3()
        ->bold(true)
        ->build();
    list_hdr->set_margin(0, 0, 0, 8);
    m_layout->add_view(list_hdr);
    ...
    m_boxes_container->set_margin(0, 0, 0, 16);
...
void TestbedView::rebuild_boxes_list() {
    ...
    if (m_info.boxes.empty()) {
        ...
        auto msg = TextViewBuilder::create()
            ->text("No testbeds created yet. Click '+ Create Arch Testbed' above to spin up a clean Arch Linux container for safe app testing.")
            ->caption()
            ->muted()
            ->multiline(true)
            ->build();
        col->add_view(msg);
        empty_card->add_view(col);
    ...
    for (size_t i = 0; i < m_info.boxes.size(); ++i) {
        ...
        auto icon_tv = TextViewBuilder::create()->text("📦")->h2()->build();
        icon_tv->set_margin(0, 0, 12, 0);
        row_top->add_view(icon_tv);
        ...
        auto desc_tv = TextViewBuilder::create()->text(desc)->caption()->muted()->ellipsize(true)->build();
        ...
        box_col->add_view(DividerViewBuilder::create()->margin(0, 10)->build());
        ...
        auto input_pkg = EditTextBuilder::create()
            ...
            ->build();
        input_pkg->set_layout_params(LayoutParams(0, 36, 1.0f));
        input_pkg->set_margin(0, 0, 8, 0);
        ...
        auto btn_install = ButtonBuilder::create()
            ...
            ->padding(12, 6)
            ->build();
        btn_install->set_margin(0, 0, 6, 0);
        ...
        if (box.installed_packages.empty()) {
            ...
            for (size_t j = 0; j < box.installed_packages.size(); ++j) {
                ...
                auto a_icon = TextViewBuilder::create()->text("⚡")->build();
                a_icon->set_margin(0, 0, 8, 0);
                app_row->add_view(a_icon);
                ...
                auto a_ver = TextViewBuilder::create()->text("Version: " + pkg.version)->caption()->muted()->build();
                ...
                if (j + 1 < box.installed_packages.size()) {
                    apps_list->add_view(DividerViewBuilder::create()->margin(0, 2)->build());
                }
            }
