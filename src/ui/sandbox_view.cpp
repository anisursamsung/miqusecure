...
SandboxView::SandboxView(const LsmInfo& info) : m_info(info) {
    ...
    // =========================================================================
    // 1. KERNEL POSTURE HERO (Direct Placement)
    // =========================================================================
    ...
    auto hero_badge = ui::make_icon_badge("security-high", auto_cfg->colors.primary_container, 34, 8, 14);
    ...
    m_stack_lbl = TextViewBuilder::create()
        ...
        ->ellipsize(false)
        ->build();
    m_stack_lbl->set_margin(0, 2, 0, 0);
    ...
    m_layout->add_view(list_col);

    // =========================================================================
    // 4. "WHAT IS SANDBOXING?" (Direct Placement Callout)
    // =========================================================================
    ...
    auto g_desc = TextViewBuilder::create()
        ->text("Confines applications inside isolated boundaries so untrusted or compromised programs cannot access personal files, devices, or network without permission.")
        ->caption()
        ->muted()
        ->multiline(true)
        ->ellipsize(false)
...
void SandboxView::rebuild_flatpak_section() {
    ...
    if (!m_info.flatpak_installed) {
        ...
        col->add_view(t);
        col->add_view(d);
        m_flatpak_container->add_view(col);
    } else {
        auto sec_fp_hdr = ui::make_section_header("SANDBOXED APPLICATIONS (" + std::to_string(m_info.flatpak_apps.size()) + ")");
        sec_fp_hdr->set_margin(0, 18, 0, 12);
        m_flatpak_container->add_view(sec_fp_hdr);
        ...
        if (m_info.flatpak_apps.empty()) {
            ...
            for (size_t i = 0; i < m_info.flatpak_apps.size(); ++i) {
                ...
                a_row->set_margin(0, 10, 0, 10);

                std::string app_icon = !miqu::ImageView::resolve_icon_path(app.id).empty() ? app.id : "package-x-generic";
                auto a_badge = ui::make_icon_badge(app_icon, cfg->colors.surface_variant, 34, 8, 14);
            ...
            m_flatpak_container->add_view(list_col);
        }
    }
}
